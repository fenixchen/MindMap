...
def fill_test_data(osd):
    ...
    osd._palettes.append(palette1)

    palette2 = Palette("调色板2", PixelFormat.LUT_8_BIT, [0xFFFF00] * 256)
    osd._palettes.append(palette2)
    ...
    osd._windows.append(window4)

    # Modifier 初始化
    move = Move("移动1", MoveDirection.NORTH, 5)
    move.link(window2)
    osd._modifiers.append(move)

    resize = Resize("大小1", 5)
    resize.link(window3)
    osd._modifiers.append(resize)

    rotate = Rotate("旋转1", 45)
    rotate.link(block1)
    osd._modifiers.append(rotate)

    slide = Slide("图片更换")
    slide.link(block_ani)
    osd._modifiers.append(slide)
